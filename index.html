...
<head>
  ...
  <title>Сравнение данных по годам</title>
...
<body>
  <div class="container">
    <header>
      <div>
        <h1>СРАВНЕНИЕ ДАННЫХ ПО ГОДАМ 📊📈</h1>
        <p class="lead">Загрузите два файла Excel за разные годы для сравнения показателей</p>
      </div>
      <div class="dropdown">
        <button class="btn btn--danger" id="menuBtn">МЕНЮ ССЫЛОК</button>
        <div class="dropdown-content" id="menuDropdown">
          <a href="https://tg-alterra.ru/" class="menu-link" target="_blank">АЛЬТЕРРА</a>
          <a href="https://znakooo.ru/" class="menu-link" target="_blank">ЗНАК</a>
          <a href="https://bar.saturn.net/" class="menu-link" target="_blank">САТУРН</a>
          <a href="https://barnaul.lemanapro.ru/" class="menu-link" target="_blank">ЛЕМАНА ПРО</a>
          <a href="#" class="menu-link" target="_blank">Ссылка 5</a>
          <a href="#" class="menu-link" target="_blank">Ссылка 6</a>
        </div>
      </div>
      <button class="btn btn--accent" id="shareCalcBtn">Расчёт доли</button>
      <div class="month-range-selector" id="monthRangeSelector" style="display: none;">
        <span class="month-range-label">Месяцы:</span>
      ...
      <div id="shareCalcModal" style="display:none;position:fixed;top:0;left:0;width:100%;height:100%;background:rgba(0,0,0,0.5);z-index:10000;justify-content:center;align-items:center;">
        <div style="background:white;padding:20px;border-radius:8px;max-width:500px;width:90%;">
          <h3 style="margin-bottom:16px;">Калькулятор доли</h3>
          <div style="display:grid;grid-template-columns:1fr 1fr;gap:12px;margin-bottom:16px;">
            <div>
              <label style="display:block;margin-bottom:4px;font-size:13px;color:var(--muted);">Часть</label>
              <input type="number" id="sharePart" style="width:100%;padding:8px;border-radius:4px;border:1px solid rgba(15,23,42,0.1);">
            </div>
            <div>
              <label style="display:block;margin-bottom:4px;font-size:13px;color:var(--muted);">Целое</label>
              <input type="number" id="shareWhole" style="width:100%;padding:8px;border-radius:4px;border:1px solid rgba(15,23,42,0.1);">
            </div>
          </div>
          <button id="shareCalcSubmit" class="btn btn--accent" style="width:100%;margin-bottom:12px;">Рассчитать</button>
          ...
          <button id="shareCalcClose" class="btn btn--outline" style="width:100%;margin-top:12px;">Закрыть</button>
        </div>
      </div>
</header>
    <div id="progressSection" class="progress-container" style="display: none;">
      <div class="progress-info">
        <div class="progress-title">Прогресс текущего года к предыдущему</div>
        <div id="progressMainValue" class="progress-value">—</div>
      </div>
      <div class="progress-details">
        <div class="progress-detail">
          <div class="progress-label">Текущий год</div>
          <div id="progressCurrentYear" class="progress-number">0</div>
        </div>
        <div class="progress-detail">
          <div class="progress-label">Предыдущий год</div>
    ...
    <div class="grid">
      <div class="panel">
        <div class="controls">
          <div class="year-comparison">
            <div class="year-tab active" data-year="current">ТЕКУЩИЙ ГОД</div>
            <div class="year-tab" data-year="previous">ПРЕДЫДУЩИЙ ГОД</div>
            <div class="year-tab" data-year="compare">СРАВНЕНИЕ</div>
          </div>
          <div class="file-row">
            <label for="file1" class="muted">Файл текущего года</label>
            <input id="file1" type="file" accept=".xlsx,.xls,.csv" />
            <label for="file1" class="file-btn pulse" title="Выберите файл Excel">Выбрать файл 1</label>
          </div>
          <div class="file-row" style="margin-top: 16px;">
            <label for="file2" class="muted">Файл предыдущего года</label>
            <input id="file2" type="file" accept=".xlsx,.xls,.csv" />
            <label for="file2" class="file-btn pulse" title="Выберите файл Excel">Выбрать файл 2</label>
          </div>
          <div class="sheet-selector" id="sheetSelector" style="display:none; margin-top: 16px;">
            <div class="muted">Выберите вкладку для анализа</div>
            <div class="sheet-options" id="sheetOptions"></div>
          </div>
          <div style="margin-top: 16px;">
            <label class="muted">Подгруппы (фильтр)</label>
            <input id="subgroupSearch" type="text" placeholder="поиск подгрупп" class="glow-pulse" style="width:100%;padding:10px;margin-top:6px;border-radius:6px;border:2px solid rgba(15,23,42,0.08);font-size:13px" />
            ...
            <div id="itemCount" class="muted" style="margin-top:8px;">Количество наименований: 0</div>
          </div>
          <div style="margin-top: 16px;">
            <label class="muted">Поставщики (фильтр)</label>
            <input id="supplierSearch" type="text" placeholder="поиск поставщиков" class="glow-pulse" style="width:100%;padding:10px;margin-top:6px;border-radius:6px;border:2px solid rgba(15,23,42,0.08);font-size:13px" />
          ...
          <div style="margin-top: 16px;">
            <label class="muted">Минимальная доля (%)</label>
            ...
            <div class="muted" style="margin-top:6px;font-size:12px">Фильтр: оставить строки, где доля хотя бы в одном месяце ≥ указанной</div>
          </div>
          <div style="display:flex;gap:10px;flex-wrap:wrap;margin-top:12px">
            <button id="selectAll" class="btn btn--accent small">Выбрать все</button>
            <button id="deselectAll" class="btn btn--outline small">Снять выбор</button>
          </div>
          <div style="display:flex;gap:10px;margin-top:16px;align-items:center;flex-wrap:wrap">
            ...
            <select id="viewMode" style="min-width:160px;flex:1;padding:10px;border-radius:6px;border:2px solid rgba(15,23,42,0.08);font-size:13px">
              <option value="group">Сумма по подгруппам</option>
              <option value="product">По товарам</option>
            </select>
          </div>
          <div style="display:flex;gap:10px;margin-top:16px;align-items:center;flex-wrap:wrap">
            <button id="stackToggle" class="btn btn--glass small">Стек / Групп</button>
            <button id="downloadChart" class="btn btn--vivid small">Скачать</button>
          </div>
          <div style="margin-top:16px;display:flex;gap:10px;flex-wrap:wrap">
            <button id="exportCSV" class="btn btn--success">Экспорт</button>
            <button id="resetBtn" class="btn btn--danger">Сброс</button>
          </div>
          <div style="margin-top:20px;">
            <div class="muted">Поиск по таблице</div>
            <input id="globalSearch" type="text" placeholder="Найти код, наименование, подгруппу..." class="glow-pulse" style="width:100%;padding:10px;margin-top:6px;border-radius:6px;border:2px solid rgba(15,23,42,0.08);font-size:13px" />
          </div>
          <div style="display:flex;gap:10px;margin-top:20px;align-items:center;flex-wrap:wrap">
            <label class="muted">Строк на странице</label>
          ...
          <div style="margin-top:20px;" class="muted">Информация</div>
          <div id="message">Загрузите файлы Excel, чтобы начать.</div>
      ...
      <div class="panel table-panel">
        <div class="table-head">
          <div class="search">
            <div class="chip">Текущий вид: <span id="currentView">Сумма по подгруппам</span></div>
            <div class="muted">/</div>
            <div class="muted">Элементов: <strong id="countLabel">0</strong></div>
          </div>
          <div class="actions">
            <button id="prevPage" class="btn btn--outline small">‹</button>
            <div class="muted" id="pageInfo" style="padding:0 12px;font-weight:600">1 / 1</div>
            <button id="nextPage" class="btn btn--outline small">›</button>
          </div>
        </div>
        ...
        <div class="chart-selector">
          <div class="chart-tab active" data-chart="main">Основной</div>
          <div class="chart-tab" data-chart="pareto">Парето</div>
          <div class="chart-tab" data-chart="heatmap">Тепловая карта</div>
        </div>
        <div style="display: flex; gap: 16px; margin-top: 16px; flex: 2;">
          ...
          <div style="height:400px; background:var(--card); padding:12px; border-radius:8px; border:1px solid rgba(15,23,42,0.06); flex: 1;">
            <h3 style="font-size:14px; margin-bottom:12px; text-align:center;">Прогресс по годам</h3>
    ...
    <div style="max-width:1200px;margin:18px auto;display:grid;grid-template-columns:1fr;gap:18px">
      <div class="panel">
        <div class="right-grid">
          <div class="summary">
            <div>
              <div class="muted">Выбранный элемент</div>
              <div id="detailTitle">—</div>
              <div class="muted" id="detailSub">—</div>
            </div>
            <div style="margin-left:auto">
              <div class="muted">Общая сумма</div>
          ...
          <div class="details">
            ...
            <div style="display:flex;gap:10px;margin-top:12px;flex-wrap:wrap">
              <button id="clearSelection" class="btn btn--outline small">Сброс выбора</button>
            </div>
          </div>
          <div class="muted">Подсказки</div>
          <ul style="margin:0 0 0 18px;color:var(--muted);font-size:13px">
            <li>Клик по заголовку столбца — сортировка.</li>
            <li>Клик по строке — детальный просмотр товара.</li>
            <li>Переключайтесь между вкладками "Текущий год", "Предыдущий год" и "Сравнение".</li>
            <li>В режиме сравнения зелёный цвет означает рост, красный — снижение показателей.</li>
            <li>Во вкладке "Парето" — анализ 80/20. В "Тепловой карте" — аномалии по цвету.</li>
          </ul>
        </div>
      </div>
    </div>
...
<script src="https://cdn.jsdelivr.net/npm/chartjs-plugin-datalabels@2.0.0"></script>
    ...
    (function(){
      ...
      if (shareCalcSubmit) {
        shareCalcSubmit.addEventListener('click', () => {
          ...
          if (isNaN(part) || isNaN(whole) || whole === 0) {
            shareCalcResult.textContent = 'Введите корректные значения';
            shareCalcResult.style.display = 'block';
            return;
          }
          const share = (part / whole * 100).toFixed(2);
          shareCalcResult.textContent = `Доля: ${share}%`;
          shareCalcResult.style.display = 'block';
        });
      }
      if (shareCalcModal) {
          ...
          options: {
                ...
                callbacks: {
                  label: function(context) {
                    if (context.datasetIndex === 0) {
                      return `Объём: ${context.raw.toLocaleString('ru-RU')}`;
                    } else {
                      return `Накоплено: ${context.raw.toFixed(1)}%`;
                    }
                  }
                }
              }
            ...
            scales: {
              ...
              y1: {
                ...
                title: { display: true, text: 'Накопленный %' }
              }
            }
          }
        });
      ...
      function resetState(){
        ...
        page = 1; 
        message.textContent = 'Загрузите файлы Excel, чтобы начать.'; 
        ...
        currentViewLabel.textContent = 'Сумма по подгруппам'; 
      ...
      function updateProgressInfo() {
        ...
        if (previousTotal === 0) {
          progressMainValue.textContent = 'Нет данных';
        } else {
      ...
      // === ИСПРАВЛЕНО: автоматический выбор первой вкладки ===
      fileInput1.addEventListener('change', async (e) => {
        ...
        try {
          ...
          const firstBtn = sheetOptions.querySelector('.sheet-option');
          if (firstBtn) {
            firstBtn.classList.add('active');
          }
          message.textContent = `Файл текущего года загружен. Выберите вкладку для анализа.`;
          document.querySelector('label[for="file1"]').classList.remove('pulse');
          // 🔥 Принудительно обновляем
          updateSelectedSheets();
        } catch(err) { 
          message.textContent = 'Ошибка чтения файла текущего года: '+(err.message||err); 
        }
      });
      fileInput2.addEventListener('change', async (e) => {
        ...
        try {
          ...
          message.textContent = `Файл предыдущего года загружен.`;
          document.querySelector('label[for="file2"]').classList.remove('pulse');
          // 🔥 Если оба файла загружены — обновляем
          if(currentWorkbook && previousWorkbook) {
            updateSelectedSheets();
          }
        } catch(err) { 
          message.textContent = 'Ошибка чтения файла предыдущего года: '+(err.message||err); 
        }
      });
      function updateSelectedSheets() {
        if(!currentWorkbook) return;
        // 🔥 Если нет активных — выбираем первую
        const activeButtons = document.querySelectorAll('.sheet-option.active');
        if (activeButtons.length === 0) {
          const firstBtn = document.querySelector('.sheet-option');
          if (firstBtn) {
            firstBtn.classList.add('active');
          }
        }
        ...
        // 💡 Всегда отображаем выбранную вкладку
        sheetInfo.textContent = `Выбрана вкладка: ${selectedSheets.join(', ')}`;
        parseSelectedSheets();
      }
      ...
      function parseSelectedSheets() {
        ...
        previousYearItems = [];
        months = [];
        // 🔥 Используем первую вкладку, если ничего не выбрано
        let sheetsToUse = selectedSheets.length > 0 ? selectedSheets : [currentWorkbook.SheetNames[0]];
        ...
        if(currentYearItems.length === 0) {
          message.textContent = 'Нет данных в выбранной вкладке';
          return;
        }
        populateFilters(); 
        page = 1; 
        ...
        message.textContent = `Загружено ${currYearCount} строк текущего года и ${prevYearCount} строк предыдущего года.`;
      ...
      function parseTable(table, sheetName, targetArray) {
        ...
        if(months.length === 0) {
          months = header.slice(4).map(h => h ? String(h) : 'Мес'); 
        }
        for(let r = 1; r < table.length; r++) {
          ...
          const code = row[0] ?? ''; 
          const name = row[1] ?? ''; 
          const subgroup = (row[2] ?? '') || '(Без подгруппы)';
          const supplier = (row[3] ?? '') || '(Без поставщика)';
          const sales = [];
      ...
      function populateFilters(){ 
        ...
        [...currentYearItems, ...previousYearItems].forEach(it => {
          subgroups.add(it.subgroup || '(Без подгруппы)');
          suppliers.add(it.supplier || '(Без поставщика)');
        });
      ...
      function updateItemCount() {
        ...
        itemCount.textContent = `Количество наименований: ${totalItems}`;
      }
        ...
        if(currentMode === 'compare') {
          ...
          if(mode === 'group'){
            ...
            for(const it of sourceItems){ 
              ...
              const key = it.subgroup || '(Без подгруппы)'; 
      ...
      function renderTableProducts(activeMonths){ 
        ...
        tbody.innerHTML=''; 
        const tr = document.createElement('tr'); 
        addTh(tr,'Код','code',true); 
        addTh(tr,'Наименование','name',true); 
        addTh(tr,'Подгруппа','subgroup',true); 
        addTh(tr,'Поставщик','supplier',true); 
        addTh(tr,'Вкладка','sheet',false);
        for(let i=0;i<activeMonths.length;i++) {
          addTh(tr, activeMonths[i], 'm'+i, true);
          addTh(tr, activeMonths[i] + ' (%)', 'p'+i, true);
        }
        thead.appendChild(tr); 
        countLabel.textContent = filtered.length; 
        currentViewLabel.textContent = 'По товарам'; 
      }
      function renderComparisonGroup(activeMonths) {
        ...
        addTh(tr, 'Подгруппа', 'group', true);
        for(let i = 0; i < activeMonths.length; i++) {
          addTh(tr, `${activeMonths[i]} (тек.)`, 'm'+i, true);
          addTh(tr, `${activeMonths[i]} (пред.)`, 'p'+i, true);
          addTh(tr, 'Разница', 'd'+i, true);
          addTh(tr, 'Изменение %', 'c'+i, true);
        }
        thead.appendChild(tr);
        countLabel.textContent = filtered.length;
        currentViewLabel.textContent = 'Сравнение по подгруппам';
      }
      function renderComparisonProducts(activeMonths) {
        ...
        thead.appendChild(tr);
        countLabel.textContent = filtered.length;
        currentViewLabel.textContent = 'Сравнение по товарам';
      }
      ...
      function showProductDetail(it) { 
        detailTitle.textContent = `${it.code} — ${it.name}`; 
      ...
      function showGroupDetail(row) { 
        ...
        detailSub.textContent = 'Подгруппа — суммарно'; 
      ...
      function showComparisonGroupDetail(row) {
        detailTitle.textContent = row.group;
        detailSub.textContent = 'Сравнение по годам';
      ...
      function showComparisonProductDetail(row) {
        detailTitle.textContent = `${row.code} — ${row.name}`;
      ...
      stackToggle.addEventListener('click', () => { 
        stacked = !stacked; 
        stackToggle.textContent = stacked ? 'Стек: ВКЛ' : 'Стек: ВЫКЛ'; 
        sortAndPaginate(); 
      });
